...
const browserKey = "guild-name-forge-browser-id";
const votesKey = "guild-name-forge-voted-name-ids";
...
function getStoredVotes() {
  try {
    return new Set<number>(JSON.parse(localStorage.getItem(votesKey) || "[]"));
  } catch {
...
export function Forge() {
  ...
  const persistVotes = useCallback((ids: Set<number>) => {
    localStorage.setItem(votesKey, JSON.stringify([...ids]));
    setVotedIds(ids);
  }, []);
  ...
  const refresh = useCallback(async () => {
    ...
    const [namesResult, votesResult] = await Promise.all([
      ...
      supabase.rpc("get_browser_vote_ids", { p_browser_id: browserId }),
    ]);
    ...
    const remoteVotes = new Set<number>((votesResult.data ?? []).map((row: { guild_name_id: number }) => row.guild_name_id));
    const merged = new Set([...getStoredVotes(), ...remoteVotes]);
    persistVotes(merged);
    ...
    setStatus("ready");
    setMessage("");
  }, [persistVotes, supabase]);

  useEffect(() => {
    const initialRefresh = window.setTimeout(() => void refresh(), 0);
    const sync = (event: StorageEvent) => {
      if (event.key === votesKey) setVotedIds(getStoredVotes());
    };
  ...
  const applyMutation = (result: Mutation, completedAt: number) => {
    ...
    const nextVoted = new Set(votedIds).add(result.guild_name_id);
    persistVotes(nextVoted);
    setNames((current) => {
  ...
  const suggest = async (event: FormEvent) => {
    ...
    setPending("suggest");
    setMessage("");
    const { data, error } = await supabase.rpc("suggest_guild_name", { p_display_name: displayName, p_browser_id: getBrowserId() });
    setPending(null);
  ...
  const vote = async (id: number) => {
    if (votedIds.has(id) || pending !== null || !supabase) return;
    setPending(id);
    setMessage("");
    const { data, error } = await supabase.rpc("vote_for_guild_name", { p_guild_name_id: id, p_browser_id: getBrowserId() });
    setPending(null);
  ...
  return <main className="page-shell">
    ...
    <section className="panel suggestion-panel" aria-labelledby="anvil-title">
      ...
      <form onSubmit={suggest}>
        ...
        <input id="guild-name" value={name} onChange={(event) => setName(event.target.value)} maxLength={60} placeholder="Enter a guild name..." disabled={pending === "suggest"} />
        <button className="primary" type="submit" disabled={pending === "suggest"}>{pending === "suggest" ? "Forging..." : "Suggest name"}</button>
      </form>
      <p className="helper">Same name, different spacing or capitalization? It becomes a vote for the existing entry.</p>
    ...
    <section className="panel rankings" aria-labelledby="rankings-title" aria-busy={status === "loading"}>
      ...
      <div className="list">
        ...
        {status === "ready" && names.map((entry, index) => <article className={`name-row ${bumpedId === entry.id ? "bump" : ""}`} key={entry.id}>
          ...
          <button className={`vote ${votedIds.has(entry.id) ? "voted" : ""}`} onClick={() => vote(entry.id)} disabled={votedIds.has(entry.id) || pending !== null}>
            {pending === entry.id ? "Voting..." : votedIds.has(entry.id) ? "Voted ✓" : "↑ Vote"}
          </button>
        </article>)}
      </div>
      <footer><span>{names.length} {names.length === 1 ? "contender" : "contenders"} · {totalVotes} total votes</span><span>{updatedLabel}</span></footer>
    </section>
    <p className="footer-note">One vote per browser, per name. No login, no loot council.</p>
  </main>;
}
